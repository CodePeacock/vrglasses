...
class FaceModel3D:
    ...
    # get head 3D model
    def __getLandmarkList(
        self, landmark_68="./project_tools/3d_model.npy", k_means=False
    ):
    ...
    def __updateModelIndexes(self):
        ...
        for idx in self.__ID_list:
            if idx == self.MODE_AXIS:
                self.__modelIndexes[self.MODE_AXIS] = range(
                    index, index + self.COUNT_AXIS
                )
                index += self.COUNT_AXIS
            elif idx == self.MODE_BOX:
                self.__modelIndexes[self.MODE_BOX] = range(
                    index, index + self.COUNT_BOX
                )
                index += self.COUNT_BOX
            elif idx == self.MODE_GLASSES:
                self.__modelIndexes[self.MODE_GLASSES] = range(
                    index, index + self.COUNT_GLASSES
                )
                index += self.COUNT_GLASSES
            elif idx == self.MODE_LANDMARK:
                self.__modelIndexes[self.MODE_LANDMARK] = range(
                    index, index + self.COUNT_LANDMARK
                )
    ...
    def setParameter(self, settingDict=None):
        ...
        for key in settingDict:
            if key in self.__ID_list and self.__ID[key] != settingDict[key]:
    ...
    def get(self, headIndex, settingDict=None):
        ...
        if headIndex < 0 or headIndex >= self.__face_count:
            print(
                "Input error : set headIndex value between 0 to {self.__face_count-1}"
            )
            return None
    ...
    def draw(self, img, end2D):
        end2D = np.asarray(end2D, dtype=int)
        self.__img_size = img.shape
        self.__line_width = int(
            max(round(min(self.__img_size[0], self.__img_size[1]) / 200), 1)
        )
        for idx in self.__ID_list:
            if idx == self.MODE_AXIS:
                if self.__ID[idx]:
                    img = self.__drawAxis(
                        img, end2D[self.__modelIndexes[self.MODE_AXIS]]
                    )
            elif idx == self.MODE_BOX:
                if self.__ID[idx]:
                    img = self.__drawBox(img, end2D[self.__modelIndexes[self.MODE_BOX]])
            elif idx == self.MODE_GLASSES:
                fix_id = self.__fixGlasses(
                    end2D[self.__modelIndexes[self.MODE_GLASSES]],
                    end2D[self.__modelIndexes[self.MODE_LANDMARK]],
                )
                if self.__ID[idx]:
                    img = self.__drawGlasses(
                        img, end2D[self.__modelIndexes[self.MODE_GLASSES]], fix_id
                    )
                if self.__glasses_img_flag:
                    img = self.__projTheGlasses(
                        img, end2D[self.__modelIndexes[self.MODE_GLASSES]], fix_id
                    )
            elif idx == self.MODE_LANDMARK:
                if self.__ID[idx]:
                    img = self.__drawLandmark(
                        img, end2D[self.__modelIndexes[self.MODE_LANDMARK]]
                    )
        return img
    ...
    def __drawAxis(self, img, axis):
        cv2.line(
            img,
            self.__getTuple(axis[0]),
            self.__getTuple(axis[1]),
            (255, 0, 0),
            2,
            cv2.LINE_AA,
        )
    ...
    def __createGlasses(self, face):
        ...
        glasses = [
            np.array(
                [
                    eye_mid[0] + self.__glasses_offset["left"],
                    eye_mid[1] + self.__glasses_offset["top"],
                    z_h,
                ]
            )
        ]
        glasses.append(
            np.array(
                [
                    eye_mid[0] - self.__glasses_offset["right"],
                    eye_mid[1] + self.__glasses_offset["top"],
                    z_h,
                ]
            )
        ...
        glasses.append(
            np.array(
                [
                    eye_mid[0] - self.__glasses_offset["right"],
                    eye_mid[1] - self.__glasses_offset["buttom"],
                    z_h,
                ]
            )
        )
        ...
        glasses.append(
            np.array(
                [
                    eye_mid[0] + self.__glasses_offset["left"],
                    eye_mid[1]
                    + self.__glasses_offset["top"]
                    + self.__glasses_offset["temple_offset"],
                    depth,
                ]
            )
        )
    ...
    def __drawGlasses(self, img, plot, fix_id, color=(255, 0, 0)):
        ...
        if fix_id[1] is not None:
            plot[10:12] = fix_id[1]
        cv2.polylines(img, [plot[:4]], True, color, self.__line_width, cv2.LINE_AA)
        cv2.polylines(img, [plot[6:10]], True, color, self.__line_width, cv2.LINE_AA)
        cv2.line(
            img,
            self.__getTuple(np.mean(plot[[4, 5]], axis=0, dtype=int)),
            self.__getTuple(np.mean(plot[[0, 3]], axis=0, dtype=int)),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
        cv2.line(
            img,
            self.__getTuple(np.mean(plot[[6, 9]], axis=0, dtype=int)),
            self.__getTuple(np.mean(plot[[10, 11]], axis=0, dtype=int)),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
        cv2.line(
            img,
            self.__getTuple(np.mean(plot[[1, 2]], axis=0, dtype=int)),
            self.__getTuple(np.mean(plot[[7, 8]], axis=0, dtype=int)),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
    ...
    def __fixGlasses(self, glasses, face):
        ...
        if face[LEFT_FACE][0] > left_glasses[0] and glasses[4][0] > face[LEFT_FACE][0]:
            ...
            fix_id.append(None)
        if (
            right_glasses[0] > face[RIGHT_FACE][0]
            and glasses[10][0] < face[RIGHT_FACE][0]
        ):
            # do fix
    ...
    def setGlassesModel(self, path=None, setup=True):
        ...
        if path is not None:
            ...
            self.__glasses = self.__modelResize(
                temp[
                    :,
                    self.__glasses_info["location"]["left"] : self.__glasses_info[
                        "location"
                    ]["right"],
                    :,
                ]
            )
            self.__glasses_temple_left = self.__modelResize(
                temp[:, : self.__glasses_info["location"]["left"], :]
            )
            self.__glasses_temple_right = self.__modelResize(
                temp[:, self.__glasses_info["location"]["right"] :, :]
            )
        self.__glasses_temple_flag = (
            self.__glasses_temple_left is not None
            and self.__glasses_temple_right is not None
        )
        self.__glasses_img_flag = False if self.__glasses is None else setup

    def __projTheGlasses(self, img, glasses_proj, fix_id):
        if self.__glasses_temple_flag:
            img = self.__perspectiveTransform(
                img,
                self.__glasses_temple_left,
                np.float32(
                    [glasses_proj[4], glasses_proj[5], glasses_proj[0], glasses_proj[3]]
                ),
                [fix_id[0], None],
            )
            img = self.__perspectiveTransform(
                img,
                self.__glasses_temple_right,
                np.float32(
                    [
                        glasses_proj[6],
                        glasses_proj[9],
        ...
        img = self.__perspectiveTransform(
            ...
            self.__glasses,
            np.float32(
                [glasses_proj[0], glasses_proj[3], glasses_proj[6], glasses_proj[9]]
            ),
            [None, None],
        )
        return img
    ...
    def __perspectiveTransform(self, img, object_img, object_proj, fix_id):
        ...
        object_Trans = cv2.warpPerspective(
            object_img, M, (self.__img_size[1], self.__img_size[0])
        )
        max_plot = np.asarray(np.max(object_proj, axis=0), dtype=int)
        min_plot = np.asarray(np.min(object_proj, axis=0), dtype=int)
        # fix with id
        if fix_id[1] is not None:
        ...
        # calculate
        Alpha = (
            object_Trans[min_plot[1] : max_plot[1], min_plot[0] : max_plot[0], 3:4]
            / 255
        )
        img[min_plot[1] : max_plot[1], min_plot[0] : max_plot[0], :] = np.asarray(
            object_Trans[min_plot[1] : max_plot[1], min_plot[0] : max_plot[0], :3]
            * Alpha
            + img[min_plot[1] : max_plot[1], min_plot[0] : max_plot[0], :]
            * (1 - Alpha),
            np.uint8,
        )
        return img
    ...
    def __modelResize(self, model):
        ...
        model = cv2.resize(
            model, (int(size_x / 5), int(size_y / 5)), interpolation=cv2.INTER_AREA
        )
        return model
    ...
    def __createBox(self, face):
        ...
        point_3d = [
            (face[0][0], face[4][1], face[25][2]),
            (face[8][0], face[4][1], face[25][2]),
            (face[8][0], top, face[25][2]),
            (face[0][0], top, face[25][2]),
            (face[0][0], face[4][1], depth),
            (face[8][0], face[4][1], depth),
            (face[8][0], top, depth),
            (face[0][0], top, depth),
        ]
        return np.asarray(point_3d, dtype=float).reshape(-1, 3)

    def __drawBox(self, img, plot, color=(0, 255, 0)):
        cv2.polylines(
            img, [plot[:4]], True, (255, 0, 0), self.__line_width, cv2.LINE_AA
        )
        cv2.polylines(img, [plot[4:8]], True, color, self.__line_width, cv2.LINE_AA)
        cv2.line(
            img,
            self.__getTuple(plot[0]),
            self.__getTuple(plot[4]),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
        cv2.line(
            img,
            self.__getTuple(plot[1]),
            self.__getTuple(plot[5]),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
        cv2.line(
            img,
            self.__getTuple(plot[2]),
            self.__getTuple(plot[6]),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
        cv2.line(
            img,
            self.__getTuple(plot[3]),
            self.__getTuple(plot[7]),
            color,
            self.__line_width,
            cv2.LINE_AA,
        )
