...
class headPose:
    ...
    def updateParameter(
        self, model_points=None, camera_matrix=None, dist_coeffs=None, point_3d=None
    ):
    ...
    def updateCameraMatrixWithSize(self, size):
        ...
        self.__camera_matrix = np.array(
            [[size[1], 0, center[0]], [0, size[1], center[1]], [0, 0, 1]],
            dtype="double",
        )
    ...
    def updateBoxSize(self, size=350, withFace=False):
        ...
        if withFace:
            ...
            front = int(size)
            point_3d = [
                (-rear, -rear, rear_depth),
                (-rear, rear, rear_depth),
                (rear, rear, rear_depth),
                (rear, -rear, rear_depth),
                (front, -front, front_depth),
                (front, front, front_depth),
                (-front, front, front_depth),
                (-front, -front, front_depth),
            ]
            point_3d = np.asarray(point_3d, dtype=float).reshape(-1, 3)
            self.__point_3d = point_3d
    ...
    def run(
        self, img, image_points, updateCamM=False, color=(0, 255, 255), withNp=False
    ):
        """
        ...
        if withNp:
            ...
            point_3d = np.append(
                self.__point_3d, np.ones((len(self.__point_3d), 1)), axis=1
            ).T
            ...
            return (
                rotationMatrixToEulerAngles(Rotation),
                self.__camera_matrix,
                Matrix,
                end,
            )
        else:
            (point_2d, jacobian) = cv2.projectPoints(
                self.__point_3d,
                rotation_vector,
                translation_vector,
                self.__camera_matrix,
            ...
            cv2.polylines(img, [point_2d[:4]], True, color, line_width, cv2.LINE_AA)
            ...
            cv2.polylines(
                img, [point_2d[4:8]], True, (0, 0, 255), line_width, cv2.LINE_AA
            )
            return img
